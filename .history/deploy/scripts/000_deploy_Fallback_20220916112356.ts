...
const func:DeployFunction = async function (hre:HardhatRuntimeEnvironment) {
    ...
    (await res).wait();
    console.log(">>>> user balance ",(await ethers.provider.getBalance(fallbackAddress)).toString());
    console.log(">>>> owner ",await fallback.owner());
    const withdrawRes = await fallback.withdraw();
    withdrawRes.wait();
    console.log(">>> withdraw",(await ethers.provider.getBalance(fallbackAddress)).toString());
